...
class ForwardThread:
	...
	@classmethod
	def put_blacklist(cls, request: BlackListForwardRequest) -> NoReturn:
		cls.put(ForwardRequest.from_super(configure.get_instance().blacklist, request))

	@classmethod
	def put(cls, request: ForwardRequest) -> NoReturn:
...
class SetTypingCoroutine:
	def __init__(self, client: Client, chat_id: int):
		...
		self.future: asyncio.Future = self.start()
...
class GetHistoryCoroutine:
	...
	async def run(self) -> NoReturn:
		...
		if not self.dirty_run:
			await self.client.send_message(self.configure.query_photo, 'Begin {} forward'.format(self.target_id))
			await self.client.send_message(self.configure.query_video, 'Begin {} forward'.format(self.target_id))
			await self.client.send_message(self.configure.query_doc, 'Begin {} forward'.format(self.target_id))
...
class BotControler:
	...
	async def create(cls, config: ConfigParser):
		self = cls(config)
		self.checker = await checkfile.create(config.get('mysql', 'host'), config.get('mysql', 'username'), config.get('mysql', 'passwd'), config.get('mysql', 'database'))
		self.redis = await aioredis.create_pool('redis://localhost')
		self.ForwardThread = ForwardThread()
		await self.redis.sadd(f'{self.redis_prefix}for_bypass', *self.checker.query_all_bypass())
		await self.redis.sadd(f'{self.redis_prefix}for_blacklist', *self.checker.query_all_blacklist())
		await self.redis.mset(self.checker.query_all_special_forward())
		await self.redis.sadd(f'{self.redis_prefix}for_admin', *self.checker.query_all_admin())
	...
	async def clean(self):
		...
		await self.redis.delete(' '.join(map(str, (key for key, _ in self.checker.query_all_special_forward().items()))))
	...
	async def reply_checker_and_del_from_blacklist(self, client: Client, msg: Message) -> NoReturn:
		try:
			pending_del = None
			...
			if pending_del is not None:
				...
				await client.send_message(self.owner_group_id, 'Remove `{}` from blacklist'.format(group_id), parse_mode='markdown')
		except:
			if msg.reply_to_message.text: print(msg.reply_to_message.text)
			logger.exception('Catch!')

	async def add_black_list(self, user_id: Union[int, dict], post_back_id=None) -> NoReturn:
		if isinstance(user_id, dict):
			await self.app.send_message(self.owner_group_id, 'User id:`{}`\nFrom chat id:`{}`\nForward from id:`{}`'.format(
		...
		if await self.redis.sadd(f'{self.redis_prefix}for_blacklist', user_id):
			await self.checker.insert_blacklist(user_id)
		logger.info('Add %d to blacklist', user_id)
		if post_back_id is not None:
			await self.app.send_message(post_back_id, 'Add `{}` to blacklist'.format(user_id),
				parse_mode='markdown')

	async def del_message_by_id(self, client: Client, msg: Message, send_message_to: Optional[Union[int, str]]=None, forward_control: bool=True) -> NoReturn:
	...
	async def get_msg_from_owner_group(self, client: Client, msg: Message) -> NoReturn:
		try:
	...
	async def forward_msg(self, msg: Message, to: int, what: str='photo') -> NoReturn:
		...
		spec_target = None if what == 'other' else await self.redis.get(msg.chat.id)
		if spec_target is None:
			#spec_target = await self.redis.get(f'{self.redis_prefix}{msg.forward_from_chat.id}')
			if msg.forward_from_chat:
				spec_target = await self.redis.get(msg.forward_from_chat.id)
	...
	async def handle_photo(self, _client: Client, msg: Message) -> NoReturn:
		await self.forward_msg(msg, self.configure.photo if self.checker.check_photo(msg.photo) else self.configure.lowq)

	async def handle_video(self, _client: Client, msg: Message) -> NoReturn:
		await self.forward_msg(msg, self.configure.video, 'video')

	async def handle_gif(self, _client: Client, msg: Message) -> NoReturn:
	...
	async def handle_other(self, _client: Client, msg: Message) -> NoReturn:
		await self.forward_msg(msg, self.configure.other, 'other')

	async def pre_private(self, client: Client, msg: Message) -> NoReturn:
		if not await self.user_checker(msg):
			await client.send(api.functions.messages.ReportSpam(peer=client.resolve_peer(msg.chat.id)))
			return
		await client.send(api.functions.messages.ReadHistory(peer=client.resolve_peer(msg.chat.id), max_id=msg.message_id))
		raise ContinuePropagation

	async def handle_add_bypass(self, _client: Client, msg: Message) -> NoReturn:
		if len(msg.text) < 4:
			return
		if await self.redis.sadd(f'{self.redis_prefix}for_bypass', msg.text[3:]):
			await self.checker.insert_bypass(msg.text[3:])
		await msg.reply('Add `{}` to bypass list'.format(msg.text[3:]), parse_mode='markdown')
		logger.info('add except id: %s', msg.text[3:])

	async def process_query(self, client: Client, msg: Message) -> NoReturn:
		r = re.match(r'^\/q (-?\d+)(d)?$', msg.text)
		if r is None:
			return
		GetHistoryCoroutine(client, msg.chat.id, r.group(1), dirty_run=r.group(2) is not None)

	async def handle_add_black_list(self, client: Client, msg: Message) -> NoReturn:
		try:
	...
	async def process_show_detail(self, _client: Client, msg: Message) -> NoReturn:
		self.echo_switch = not self.echo_switch
		await msg.reply('Set echo to {}'.format(self.echo_switch))

	async def set_forward_target_reply(self, _client: Client, msg: Message) -> NoReturn:
		...
		await self._set_forward_target(r.group(1), r1.group(1), msg)
	...
	async def set_forward_target(self, _client: Client, msg: Message) -> NoReturn:
		r = re.match(r'^\/f (-?\d+) (other|photo|bot|video|anime|gif|doc|lowq)$', msg.text)
		if r is None:
			return
		await self._set_forward_target(r.group(1), r.group(2), msg)

	async def _set_forward_target(self, chat_id: int, target: str, msg: Message) -> NoReturn:
	...
	async def add_user(self, _client: Client, msg: Message) -> NoReturn:
	...
	async def change_code(self, _client: Client, msg: Message) -> NoReturn:
		r = re.match(r'^/pw (.+)$', msg.text)
		if r:
			await msg.reply('Success changed authorize code.')

	async def undo_blacklist_operation(self, client: Client, msg: Message) -> NoReturn:
		await self.reply_checker_and_del_from_blacklist(client, msg)

	async def switch_detail2(self, _client: Client, msg: Message) -> NoReturn:
		self.custom_switch = not self.custom_switch
		await msg.reply(f'Switch custom print to {self.custom_switch}')

	async def switch_detail(self, _client: Client, msg: Message) -> NoReturn:
		self.detail_msg_switch = not self.detail_msg_switch
		await msg.reply(f'Switch detail print to {self.detail_msg_switch}')

	async def callstopfunc(self, _client: Client, msg: Message) -> NoReturn:
		#msg.reply('Exiting...')
		#Thread(target=process_exit.exit_process, args=(2,)).start()
		pass

	async def show_help_message(self, _client: Client, msg: Message) -> NoReturn:
	...
	async def process_private(self, _client: Client, msg: Message) -> NoReturn:
		...
		if r is None: return
		await self.add_black_list(r.group(1), msg.chat.id)

	async def start(self) -> NoReturn:
		await self.app.start()
		await self.ForwardThread.start()
